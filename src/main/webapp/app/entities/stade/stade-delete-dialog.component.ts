...
export class StadeDeleteDialogComponent {
    ...
    constructor(private stadeService: StadeService, public activeModal: NgbActiveModal, private eventManager: JhiEventManager) {}
...
export class StadeDeletePopupComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.route.data.subscribe(({ stade }) => {
            setTimeout(() => {
                this.ngbModalRef = this.modalService.open(StadeDeleteDialogComponent as Component, { size: 'lg', backdrop: 'static' });
                this.ngbModalRef.componentInstance.stade = stade.body;
